"""
run_v1_baseline.py – V1 one-pass baseline for ablation comparison.
Runs the same audio through a single BART call with no chunking.
Usage: python run_v1_baseline.py  (venv must be active)
Output: outputs/v1_baseline_results.json
"""
...
# 4. one-pass summarisation (no chunking)
print("\n[4/4] One-pass summarisation (V1)...")
t      = time.perf_counter()
...
# side-by-side comparison if V4 results exist
...
if os.path.exists(v4_path):
    ...
    print(fmt.format("Compression",
          f"{onepass_compression:.3f}",
          f"{v4.get('hier_compression', '—')}" if isinstance(v4.get('hier_compression'), float) else "—"))
